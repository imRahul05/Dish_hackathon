...
export default function AIWritingAssistant({ onInsert, currentContent, onUpdateTitle = () => {}, scannedFood }) {
  ...
  useEffect(() => {
    if (scannedFood) {
      generateFoodInsights(scannedFood);
    }
  }, [scannedFood]);

  const generateFoodInsights = async (food) => {
    setLoading(true);
    try {
      const prompt = `Provide detailed insights and analysis for the following dish:\n\nName: ${food.dishName}\nItems: ${food.items.map(item => `${item.name} (Quantity: ${item.quantity}, Calories: ${item.calories})`).join(', ')}`;
      const insights = await getContentImprovements(prompt);
      setFoodInsights(insights);
    } catch (error) {
                ...
                <CardContent className="p-4">
                  ...
                  {foodInsights && (
                    <Card className="mt-4">
                      ...
                      <CardContent>
                        <ScrollArea className="h-[150px] w-full rounded-md p-2 bg-gray-800">
                          <p className="text-sm leading-relaxed whitespace-pre-wrap text-gray-300">{foodInsights}</p>
                        </ScrollArea>
                      </CardContent>
                    </Card>
                  )}
